...
export default function FleetTransitView({ 
  ...
  const filteredConvoys = useMemo(() => {
    if (filterMode === 'EN_ROUTE') {
      return convoys.filter(c => c.status === 'EN ROUTE' || c.status === 'EN_ROUTE' || c.status === 'BOARDING');
    }
    if (filterMode === 'STANDBY') {
      return convoys.filter(c => c.status === 'STANDBY' || c.status === 'READY' || c.status === 'STAGED' || c.status === 'HALTED');
    }
    return convoys;
  }, [convoys, filterMode]);
  ...
  const handleAutoBalance = async () => {
    ...
    try {
      const persistPromises = updatedConvoys
        .filter(c => c.axis.includes('Ridge') || c.pickup.includes('Sunil') || c.pickup.includes('Singhdhar') || c.pickup.includes('Manohar') || c.type.includes('Helicopter'))
        .map(c => {
    ...
    setToastMsg("🔀 FLEET AUTO-BALANCED: Critical ward transport & air-corridor units dispatched.");
  ...
  return (
    <div className="w-full bg-white border border-slate-200 rounded-xl p-4 sm:p-6 font-sans text-slate-800 shadow-sm space-y-4" id="fleet-transit-view">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b border-slate-200 pb-4 gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-blue-50 text-blue-800 border border-blue-200">
                  UTC &bull; ITBP &bull; IAF &bull; NDRF
                </span>
              </div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold px-3 py-1 rounded bg-emerald-50 text-emerald-800 border border-emerald-300 font-mono flex items-center gap-1.5 shadow-2xs">
            ...
            {convoys.length} TACTICAL UNITS ACTIVE
          </span>
        </div>
      </div>
      ...
      <div className="mt-4 p-3 bg-slate-50 border border-slate-200 rounded-lg flex flex-wrap items-center justify-between gap-3 text-xs">
